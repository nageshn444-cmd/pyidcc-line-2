          ...
          <table className="w-full text-left border-collapse font-mono text-[11px] min-w-[2000px]">
            <thead>
              <tr className="bg-slate-950 border-b border-slate-800 text-center sticky top-0 z-30">
                <th className="w-[70px] py-2" rowSpan="2">Actions</th>
                ...
                const renderWttCell = (direction, stationName, isTidField = false) => {
                  ...
                  return (
                    <td key={`cell-${row.id || rowIdx}-${direction}-${stationName || 'tid'}`} 
                        onDoubleClick={() => { setEditingCell({ rowId: row.id, direction, station: stationName, isTid: isTidField }); setEditValue(baseValue); }} 
                        className={`py-2 px-1 border-r border-slate-800/30 cursor-pointer ${textColor} ${stationName === 'PYID' ? 'bg-emerald-800/30 font-bold shadow-inner' : ''} ${isTidField ? `${stickyTidBgClass} sticky left-0 border-r-2 border-slate-800 z-10` : ''}`}>
                      {cellValue}
                    </td>
                  );
                };
