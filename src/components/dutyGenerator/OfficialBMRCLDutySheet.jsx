...
import { Printer, Download, Send, X, FileSpreadsheet, CheckCircle2 } from 'lucide-react';
...
export const getDutyDisplayType = (item) => {
  ...
  const code = String(item.dutyCode || item.assignedDutyCode || '').toUpperCase();

  if (num === 1 || train.toLowerCase() === 'pro 1' || code.includes('PRO1')) return 'PRO1';
  if (num === 2 || train.toLowerCase() === 'stby' || code.includes('OR1')) return 'Stdby';
  if (sOnLoc.includes('KGWA') && (sOnLoc.includes('Dn') || code.includes('JC'))) return 'KGWA Dn';
  ...
  if (sOnLoc.includes('No PDC') || sOnLoc.includes('Depo/No PDC')) return 'No PDC';
  if (sOnLoc.includes('D-Rd3') || sOnLoc.includes('Dpo - Rd3')) return 'D-Rd3';
  if (sOnLoc.includes('PYID Dn')) return 'PYID Dn';
  if (sOnLoc.includes('PYID') && !sOnLoc.includes('Dn') && !sOnLoc.includes('Up')) return 'PYID';
  ...
  if (item.assignedDutyCode?.includes('TEST') || item.specialTag === 'TESTING') return 'Testing';
  if (item.role === 'TRAINEE' || item.assignmentCategory === 'TRAINEE') return 'Trainee';
...
export const getSignOnDisplayLocation = (item) => {
  ...
  if (s.includes('Depot') || s.includes('Depo') || s.includes('No PDC') || s.includes('Rd3')) return 'Depot';
...
export default function OfficialBMRCLDutySheet({
  ...
  const formattedTitleDate = useMemo(() => formatSheetHeaderDate(targetDate), [targetDate]);
  ...
  const leftRows = useMemo(() => {
    ...
    rows.push({ isBanner: false, type: '', sOnTime: '', sOnLoc: '', name: '', empNo: '22461', sOffTime: '', sOffLoc: '' });
    rows.push({ isBanner: false, type: '', sOnTime: '', sOnLoc: '', name: '', empNo: '22528', sOffTime: '', sOffLoc: '' });
    rows.push({ isBanner: false, type: '', sOnTime: '', sOnLoc: '', name: '', empNo: '22499', sOffTime: '', sOffLoc: '' });
  ...
  const rightSections = useMemo(() => {
    ...
    ccRows.push({ tag: 'CC1', from: cc1.sOnTime || '6:30', name: cc1.name, empNo: String(cc1.empId || cc1.empNo || '21945'), to: cc1.sOffTime || '14:00' });
    ccRows.push({ tag: 'CC2', from: cc2.sOnTime || '14:00', name: cc2.name, empNo: String(cc2.empId || cc2.empNo || '20726'), to: cc2.sOffTime || '21:30' });
    ccRows.push({ tag: 'CC3', from: cc3.sOnTime || '21:30', name: cc3.name, empNo: String(cc3.empId || cc3.empNo || '21078'), to: cc3.sOffTime || '6:30' });
    ...
    const findStbk = (stn, shift) => stationStandbyDuties.find(s => (s.stbkStation === stn || s.location === stn) && (!shift || s.shift === shift));

    const ngsa1 = findStbk('NGSA', 'A') || { name: 'Jagadeesh S', empId: 21994, sOnTime: '6:30', sOffTime: '14:00' };
    const ngsa2 = findStbk('NGSA', 'B') || { name: '', empId: '', sOnTime: '6:30', sOffTime: '14:00' };
    const puth1 = findStbk('PUTH', 'A') || { name: '', empId: '', sOnTime: '6:30', sOffTime: '14:00' };
    const puth2 = findStbk('PUTH', 'B') || { name: '', empId: '', sOnTime: '', sOffTime: '14:00' };
    const kgwa1 = findStbk('KGWA', 'A') || { name: '', empId: '', sOnTime: '14:00', sOffTime: '22:00' };
    const rvr1 = findStbk('RVR', 'A') || { name: '', empId: '', sOnTime: '14:00', sOffTime: '22:00' };
    const kgwa2 = findStbk('KGWA', 'B') || { name: '', empId: '', sOnTime: '', sOffTime: '' };
    const rvr2 = findStbk('RVR', 'B') || { name: '', empId: '', sOnTime: '', sOffTime: '' };
    ...
    outstationRows.push({ tag: 'NGSA', from: '6:30', name: ngsa1.name, empNo: String(ngsa1.empId || ''), to: '14:00' });
    outstationRows.push({ tag: 'NGSA', from: '6:30', name: ngsa2.name, empNo: String(ngsa2.empId || ''), to: '14:00' });
    outstationRows.push({ tag: 'PUTH', from: '6:30', name: puth1.name, empNo: String(puth1.empId || ''), to: '14:00' });
    outstationRows.push({ tag: 'PUTH', from: '', name: puth2.name, empNo: String(puth2.empId || ''), to: '' });
    outstationRows.push({ tag: 'KGWA', from: '14:00', name: kgwa1.name, empNo: String(kgwa1.empId || ''), to: '22:00' });
    outstationRows.push({ tag: 'RVR', from: '14:00', name: rvr1.name, empNo: String(rvr1.empId || ''), to: '22:00' });
    outstationRows.push({ tag: 'KGWA', from: '', name: kgwa2.name, empNo: String(kgwa2.empId || ''), to: '' });
    outstationRows.push({ tag: 'RVR', from: '', name: rvr2.name, empNo: String(rvr2.empId || ''), to: '' });
    outstationRows.push({ tag: 'BJET', from: '7:00', name: bjet1.name, empNo: String(bjet1.empId || ''), to: '15:00' });
    outstationRows.push({ tag: 'BJET', from: '14:00', name: bjet2.name, empNo: String(bjet2.empId || ''), to: '22:00' });
    ...
    const woRows = weekOffStaff.length >= 8 
      ? weekOffStaff.map(w => ({ from: w.sOnTime || '7:00', name: w.name, empNo: String(w.empId || w.empNo || ''), to: w.sOffTime || '15:00' }))
      : canonicalWOList;
    sections.push({ id: 'WO', tagLabel: 'Weekly Off', isVerticalTag: true, rows: woRows });
    ...
    const actualCL = leaveStaff.filter(l => (l.assignmentSubType === 'CL' || l.leaveType === 'CL'));
    const clRows = (actualCL.length >= 5 ? actualCL : canonicalCLList).map(c => ({
      from: '',
      name: c.name,
      empNo: String(c.empId || c.empNo || ''),
      to: ''
    }));
    sections.push({ id: 'CL', tagLabel: 'CL', isVerticalTag: true, rows: clRows });
    ...
    const actualEL = leaveStaff.filter(l => (l.assignmentSubType === 'EL' || l.leaveType === 'EL'));
    const elRows = (actualEL.length > 0 ? actualEL : canonicalELList).map(e => ({
      from: e.from || '10-Sep',
      name: e.name,
      empNo: String(e.empId || e.empNo || ''),
      to: e.to || '10-Sep'
    }));
    sections.push({ id: 'EL', tagLabel: 'EL', isVerticalTag: true, rows: elRows });

    // ── SECTION 6: GHEL (GOVT HOLIDAY EARNED LEAVE) ──
    const actualGHEL = leaveStaff.filter(l => l.assignmentSubType === 'GHEL' || l.leaveType === 'GHEL');
    const ghelRows = actualGHEL.length > 0 
      ? actualGHEL.map(g => ({ from: g.from || '10-Sep', name: g.name, empNo: String(g.empId || g.empNo || ''), to: g.to || '10-Sep' }))
      : [{ from: '10-Sep', name: 'Aravinda Vinod Kumar', empNo: '22284', to: '10-Sep' }];
    sections.push({ id: 'GHEL', tagLabel: 'GHEL', isVerticalTag: true, rows: ghelRows });

    // ── SECTION 7: LEAVE & ABSENT (Blank/Placeholder headers) ──
    sections.push({ id: 'LEAVE_BLANK', tagLabel: 'Leave', isVerticalTag: true, rows: [{ from: '', name: '', empNo: '', to: '' }] });
    sections.push({ id: 'ABSENT_BLANK', tagLabel: 'Absent', isVerticalTag: true, rows: [{ from: '', name: '', empNo: '', to: '' }] });

    // ── SECTION 8: ML (MEDICAL LEAVE) ──
    const actualML = leaveStaff.filter(l => l.assignmentSubType === 'ML' || l.leaveType === 'ML');
    const mlRows = actualML.length > 0
      ? actualML.map(m => ({ from: m.from || '10-Sep', name: m.name, empNo: String(m.empId || m.empNo || ''), to: m.to || '10-Sep' }))
      : [{ from: '10-Sep', name: 'Karan Velarasan', empNo: '88000048', to: '10-Sep' }];
    sections.push({ id: 'ML', tagLabel: 'ML', isVerticalTag: true, rows: mlRows });

    // ── SECTION 9: HPL (HALF PAY LEAVE) ──
    const actualHPL = leaveStaff.filter(l => l.assignmentSubType === 'HPL' || l.leaveType === 'HPL');
    const hplRows = actualHPL.length > 0
      ? actualHPL.map(h => ({ from: h.from || '29-Jul', name: h.name, empNo: String(h.empId || h.empNo || ''), to: h.to || '24-Jan' }))
      : [
          { from: '29-Jul', name: 'Chaitranjali UG', empNo: '22456', to: '24-Jan' },
          { from: '13-Aug', name: 'GA Sudhakar', empNo: '22227', to: '12-Oct' }
        ];
    sections.push({ id: 'HPL', tagLabel: 'HPL', isVerticalTag: true, rows: hplRows });
    ...
    const actualCRRC = trainingStaff.filter(t => t.specialProfile === 'CRRC' || String(t.empId).startsWith('88'));
    const crrcRows = (actualCRRC.length >= 5 ? actualCRRC : canonicalCRRCList).map(c => ({
      from: c.from || '7-Sep',
      name: c.name,
      empNo: String(c.empId || c.empNo || ''),
      to: ''
    }));
    sections.push({ id: 'CRRC_TRG', tagLabel: 'RS CRRC-DM Train 440kms Trg', isVerticalTag: true, rows: crrcRows });
    ...
    const actualPink = pinkLine4Staff.length >= 5 
      ? pinkLine4Staff.map(p => ({ from: '2-Jul', name: p.name, empNo: String(p.empId || p.empNo || ''), to: '' }))
      : canonicalPinkList;
    sections.push({ id: 'PINK_LINE', tagLabel: 'Pink Line 4', isVerticalTag: true, rows: actualPink });
    ...
    sections.push({ id: 'WHTM', tagLabel: 'Temporary WHTM', isVerticalTag: true, rows: canonicalWHTM });

    return sections;
  }, [ccDuties, stationStandbyDuties, weekOffStaff, leaveStaff, trainingStaff, pinkLine4Staff]);
  ...
  const clCount = (leaveStaff.filter(l => l.assignmentSubType === 'CL' || l.leaveType === 'CL').length) || 14;
  const elGhelCount = (leaveStaff.filter(l => ['EL', 'GHEL'].includes(l.assignmentSubType) || ['EL', 'GHEL'].includes(l.leaveType)).length) || 1;
  const jmdLCount = 2;
  const mlHplCount = (leaveStaff.filter(l => ['ML', 'HPL'].includes(l.assignmentSubType) || ['ML', 'HPL'].includes(l.leaveType)).length) || 2;
  ...
  const sheetContent = (
    <div id="bmrcl-official-print-duty-sheet" className="bg-white text-black font-sans text-[11px] leading-tight select-text print:m-0 print:p-0">
      <style>{`
        @media print {
          @page {
            size: portrait;
      ...
      <div 
        ...
        style={{ backgroundColor: '#ff4081', WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}
      >
        {formattedTitleDate}
      </div>
        ...
        <table className="w-full border-collapse text-[10.5px]">
          <thead>
            ...
            <tr className="bg-white text-black font-black border-b border-black text-center">
              ...
              <th className="border-r border-black py-1 px-1.5 w-[65px]">Type</th>
              <th className="border-r border-black py-1 px-1 w-[55px]">Sign On Time</th>
              <th className="border-r border-black py-1 px-1 w-[60px]">Sign On Location</th>
              <th className="border-r border-black py-1 px-2 text-left min-w-[140px]">NAME</th>
              <th className="border-r border-black py-1 px-1 w-[80px]">Emp No</th>
              <th className="border-r border-black py-1 px-1 w-[55px]">Sign OFF Time</th>
              <th className="border-r-2 border-black py-1 px-1 w-[60px]">Sign OFF Location</th>

              {/* RIGHT HALF HEADERS (Columns 8 to 12) */}
              <th className="border-r border-black py-1 px-1 w-[50px]">From</th>
              <th className="border-r border-black py-1 px-2 text-left min-w-[140px]">Name</th>
              <th className="border-r border-black py-1 px-1 w-[75px]">Emp.No.</th>
          ...
          <tbody>
            {Array.from({ length: maxRows }).map((_, rIdx) => {
              ...
              return (
                <tr key={rIdx} className="border-b border-black/80 hover:bg-slate-50">
                  
                  {/* ── LEFT HALF CELLS (7 COLS) ── */}
                  {leftItem ? (
                    leftItem.isBanner ? (
                      <td colSpan={7} className="border-r-2 border-black text-center font-black py-1 px-2 bg-slate-100 uppercase tracking-wider text-[11px]">
                        {leftItem.title}
                      </td>
                    ) : (
                      <>
                        <td className="border-r border-black py-0.5 px-1 font-bold text-center truncate">{leftItem.type}</td>
                        <td className="border-r border-black py-0.5 px-1 text-center font-mono">{leftItem.sOnTime}</td>
                        <td className="border-r border-black py-0.5 px-1 text-center truncate">{leftItem.sOnLoc}</td>
                        <td className="border-r border-black py-0.5 px-2 font-bold text-left truncate">{leftItem.name}</td>
                        <td className="border-r border-black py-0.5 px-1 text-center font-mono">{leftItem.empNo}</td>
                        <td className="border-r border-black py-0.5 px-1 text-center font-mono">{leftItem.sOffTime}</td>
                        <td className="border-r-2 border-black py-0.5 px-1 text-center truncate">{leftItem.sOffLoc}</td>
                      </>
                    )
                  ) : (
                    <>
                  ...
                  {rightItem ? (
                    <>
                      ...
                      <td className="border-r border-black py-0.5 px-1 text-center font-mono">{rightItem.from || ''}</td>
                      <td className="border-r border-black py-0.5 px-2 font-bold text-left truncate">{rightItem.name || ''}</td>
                      <td className="border-r border-black py-0.5 px-1 text-center font-mono">{rightItem.empNo || ''}</td>
                      <td className="py-0.5 px-1 text-center font-mono">{rightItem.to || ''}</td>
                    </>
                  ) : (
                    <>
                      <td className="border-r border-black py-0.5 px-1"></td>
      ...
      <div className="w-full border-x-2 border-b border-black grid grid-cols-11 text-center font-bold text-[10px] divide-x divide-black bg-white">
        <div className="py-1">
          <div className="text-black font-black uppercase text-[9.5px]">Present</div>
          <div className="text-blue-900 font-mono text-xs">{presentCount || 75}</div>
        </div>
        <div className="py-1">
          <div className="text-black font-black uppercase text-[9.5px]">Rest</div>
          <div className="text-slate-800 font-mono text-xs">{restCount || 19}</div>
        </div>
        <div className="py-1">
          <div className="text-black font-black uppercase text-[9.5px]">CL</div>
          <div className="text-amber-800 font-mono text-xs">{clCount || 14}</div>
        </div>
        <div className="py-1">
          <div className="text-black font-black uppercase text-[9.5px]">EL + GHEL</div>
          <div className="text-purple-800 font-mono text-xs">{String(elGhelCount).padStart(2, '0') || '01'}</div>
        </div>
        <div className="py-1">
          <div className="text-black font-black uppercase text-[9.5px]">CRT</div>
          <div className="text-slate-600 font-mono text-xs">--</div>
        </div>
        <div className="py-1">
          <div className="text-black font-black uppercase text-[9.5px]">JMD L</div>
          <div className="text-slate-800 font-mono text-xs">{String(jmdLCount).padStart(2, '0')}</div>
        </div>
        <div className="py-1">
          <div className="text-black font-black uppercase text-[9.5px]">ML&HPL</div>
          <div className="text-rose-800 font-mono text-xs">{String(mlHplCount).padStart(2, '0')}</div>
        </div>
        <div className="py-1">
          <div className="text-black font-black uppercase text-[9.5px]">L1 CC</div>
          <div className="text-indigo-800 font-mono text-xs">{l1CcCount || 3}</div>
        </div>
        <div className="py-1">
          <div className="text-black font-black uppercase text-[9.5px]">AB</div>
          <div className="text-red-700 font-mono text-xs">{String(abCount).padStart(2, '0')}</div>
        </div>
        <div className="py-1">
        ...
        <div className="py-1 bg-slate-100">
          <div className="text-black font-black uppercase text-[9.5px]">TOTAL</div>
          <div className="text-emerald-900 font-mono text-xs font-black">{totalCount} (138-20)</div>
        </div>
      </div>
      ...
      <div className="w-full border-x-2 border-b-2 border-black grid grid-cols-6 text-center text-[9.5px] font-bold divide-x divide-black py-1 bg-white">
        <div className="text-left px-2 truncate">
          <strong>Prepared By:</strong> {loggedInUserName.split('(')[0].trim() || 'Nagesh N'}
        </div>
        <div className="font-mono">
          {new Date().toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false })} hrs
        </div>
        <div>
          on: <span className="font-mono">{targetDate ? targetDate.split('-').reverse().join('-') : '10-09-2026'}</span>
        </div>
        <div className="font-mono">
          10-09-2026
        </div>
        ...
        <div className="truncate text-blue-900 font-black">
          {dayType === 'SATURDAY' ? 'Saturday Link' : dayType === 'SUNDAY' ? 'Sunday Link' : 'Weekday Link'}
        </div>
      </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400">
                Exact Control-Room format for <strong className="text-slate-200">{formattedTitleDate}</strong> ({dayType})
              </p>
            </div>
          </div>
