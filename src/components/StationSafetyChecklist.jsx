    ...
    <div className='bg-slate-900 p-6 rounded-xl border border-slate-800'>
      ...
      <div className='space-y-4 mb-6'>
        {['fireSafety', 'cctv', 'platformEdge'].map(item => (
          <label key={item} className='flex items-center gap-3 text-slate-300' htmlFor={`safety-${item}`}>
            <input id={`safety-${item}`} name={`safety_${item}`} type="checkbox" className='w-4 h-4' checked={report[item]} onChange={e => setReport({...report, [item]: e.target.checked})} />
