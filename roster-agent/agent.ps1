...
function Log([string]$m){Add-Content -Path $LogPath -Value ((Get-Date -Format 'yyyy-MM-dd HH:mm:ss')+' | '+$m) -Encoding UTF8}
...
$url=[string]$c.functionUrl;$token=[string]$c.agentToken;$poll=[int]$c.pollSeconds;if($poll -lt 30){$poll=60};$agentId=[string]$c.agentId;if([string]::IsNullOrWhiteSpace($agentId)){$agentId=$env:COMPUTERNAME}
...
function ReadRoster($path,[datetime]$date){
 ...
 try{
  ...
  $used=$sheet.UsedRange;$rows=[int]$used.Rows.Count;$cols=[int]$used.Columns.Count;$v=$used.Value2;if($rows -lt 1){throw 'Today worksheet is empty.'}
  function V($r,$col){try{if($rows -eq 1 -and $cols -eq 1){if($r -eq 1 -and $col -eq 1){return $v};return $null};return $v.GetValue($r,$col)}catch{return $null}}
  $hr=-1;$dc=1;$nc=5;$ec=6;$pc=-1;$tc=-1
  for($r=1;$r -le [Math]::Min(25,$rows);$r++){$cells=1..$cols|ForEach-Object{[string](V $r $_)};$low=$cells|ForEach-Object{$_.Trim().ToLowerInvariant()};$hd=@($low|Where-Object{$_ -like '*duty*'}).Count -gt 0;$hn=@($low|Where-Object{$_ -like '*name*' -or $_ -like '*operator*' -or $_ -eq 'to' -or $_ -eq 't.o' -or $_ -eq 't.o.'}).Count -gt 0;$he=@($low|Where-Object{$_ -like '*emp*' -or $_ -like '*employee*' -or $_ -eq 'id'}).Count -gt 0;if(($hd -and($hn -or $he))-or($hn -and $he)){$hr=$r;for($col=1;$col -le $cols;$col++){$h=([string](V $r $col)).Trim().ToLowerInvariant();if($h -like '*duty*'){$dc=$col}elseif($h -like '*train*' -or $h -like '*rake*'){$tc=$col}elseif($h -like '*name*' -or $h -like '*operator*' -or $h -eq 'to' -or $h -eq 't.o' -or $h -eq 't.o.'){$nc=$col}elseif($h -like '*emp*' -or $h -like '*employee*' -or $h -eq 'id'){$ec=$col}elseif($h -like '*pattern*' -or $h -like '*trip*'){$pc=$col}};break}}
  ...
  if($duties.Count -eq 0){throw 'No duty rows extracted.'};[pscustomobject]@{sheetName=[string]$sheet.Name;duties=@($duties)}
 }finally{if($wb){try{$wb.Close($false)}catch{};try{[Runtime.InteropServices.Marshal]::ReleaseComObject($wb)|Out-Null}catch{}};if($excel){try{$excel.Quit()}catch{};try{[Runtime.InteropServices.Marshal]::ReleaseComObject($excel)|Out-Null}catch{}};[GC]::Collect();[GC]::WaitForPendingFinalizers()}
}
function Sync{
 $now=Get-Date;$yearRoot=Join-Path $root ([string]$now.Year)
 if(-not(Test-Path $root)){Log('ERROR root missing: '+$root);return $false};if(-not(Test-Path $yearRoot)){Log('WAIT year folder missing: '+$yearRoot);return $false}
 $files=@(CandidateFiles $yearRoot $now);if($files.Count -eq 0){Log('WAIT today roster not found for '+$now.ToString('yyyy-MM-dd')+' under '+$yearRoot);return $false}
 $selected=$null;$parsed=$null
 foreach($f in $files){if(((Get-Date)-$f.LastWriteTime).TotalSeconds -lt 15){Log('WAIT file still changing: '+$f.FullName);continue};try{$p=ReadRoster $f.FullName $now;if($p.duties.Count -gt 0){$selected=$f;$parsed=$p;break}}catch{Log('SKIP invalid candidate '+$f.FullName+' | '+$_.Exception.Message)}}
 if(-not $selected){Log('WAIT no valid current-date roster workbook');return $false}
 $hash=(Get-FileHash $selected.FullName -Algorithm SHA256).Hash.ToLowerInvariant();$st=$null;if(Test-Path $StatePath){try{$st=Get-Content -Raw $StatePath|ConvertFrom-Json}catch{}}
 if($st -and $st.selectedDate -eq $now.ToString('yyyy-MM-dd') -and $st.sha256 -eq $hash -and $st.status -eq 'DEPLOYED'){Log('SKIP already deployed '+$selected.Name);return $true}
 ...
 for($a=1;$a -le 3;$a++){try{$h=@{'x-pyidcc-agent-token'=$token};$r=Invoke-RestMethod -Uri $url -Method Post -Headers $h -ContentType 'application/json' -Body $payload -TimeoutSec 120;if($r.ok -and($r.status -eq 'DEPLOYED' -or $r.status -eq 'ALREADY_DEPLOYED')){@{status='DEPLOYED';selectedDate=$now.ToString('yyyy-MM-dd');filePath=$selected.FullName;fileName=$selected.Name;sheetName=$parsed.sheetName;sha256=$hash;deployedAt=(Get-Date).ToString('o')}|ConvertTo-Json|Set-Content $StatePath -Encoding UTF8;Log('SUCCESS '+$r.status+' | '+$selected.Name+' | sheet='+$parsed.sheetName+' | duties='+$r.dutyCount);return $true};throw('Cloud response was unsuccessful: '+($r.error|Out-String))}catch{Log('RETRY '+$a+'/3 | '+$_.Exception.Message);if($a -lt 3){Start-Sleep -Seconds (10*$a)}}}
 return $false
}
Log('START mode='+($(if($Once){'ONCE'}else{'CONTINUOUS'}))+' root='+$root+' agent='+$agentId)
if($Once){[void](Sync);exit 0}
while($true){try{[void](Sync)}catch{Log('UNHANDLED '+$_.Exception.Message)};Start-Sleep -Seconds $poll}
